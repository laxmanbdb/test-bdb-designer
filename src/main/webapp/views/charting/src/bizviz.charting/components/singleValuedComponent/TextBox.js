 ...
 * @description TextBox
 **/
function TextBox(m_chartContainer, m_zIndex) {
	// console.log("inside TextBox() constructor");
	this.base = Widget;
	this.base();
...
TextBox.prototype.setProperty = function (chartJson) {
	this.ParseJsonAttributes(chartJson.Object, this);
	this.initCanvas();
};

TextBox.prototype.ParseJsonAttributes = function (jsonObject, nodeObject) {
	this.chartJson = jsonObject;
	for (var key in jsonObject) {
		if (key == "NividhTextBox") {
...
TextBox.prototype.setDataProvider = function (data) {
	this.m_text = data;
};
TextBox.prototype.initCanvas = function () {
	var temp = this;
	$("#draggableDiv" + temp.m_objectid).remove();
	this.initializeDraggableDivAndCanvas();
};

TextBox.prototype.initializeDraggableDivAndCanvas = function (dashboardName, zindex) {
	this.setDashboardNameAndObjectId();
	this.createDraggableDiv(this.m_chartContainer, this.m_zIndex);
};

TextBox.prototype.setDashboardNameAndObjectId = function () {
...
TextBox.prototype.drawObject = function () {
	this.init();
	this.drawChart();
...
TextBox.prototype.getTextValueFromGlobalVariable = function () {
	var str, key;
	str = key = this.m_text;
	var re = /\[(.*?)\]/g;
...
TextBox.prototype.drawChart = function () {
...
TextBox.prototype.getDataProvider = function () {
	return this.m_textBoxText;
};
TextBox.prototype.drawTextBox = function () {
	var temp = this;
	...
	obj.style.backgroundColor = hex2rgb(convertColorToHex(this.m_contentbackgroundcolor), this.m_contentbackgroundalpha);
	if (IsBoolean(this.m_bordervisible)) {
		obj.style.border = ""+this.m_borderthickness+"px "+ this.m_borderstyle + "" + convertColorToHex(this.m_bordercolor);
		obj.style.borderRadius = this.m_borderradius + "px";
	}
	...
	if (IsBoolean(this.m_bordervisible)) {
		box.style.top = (this.m_borderradius * 1/3) + "px";
		box.style.left = (this.m_borderradius * 1/3 ) + "px";
		box.style.width = (this.m_width - this.m_borderthickness * 2 - this.m_borderradius * 2/3) + "px";
		box.style.height = (this.m_height - this.m_borderthickness * 2 - this.m_borderradius * 2/3) + "px";
	}
	box.style.padding = this.m_textboxpadding+"px";
	box.setAttribute("contentEditable", this.m_editable);
	...
	if ( (temp1 != undefined && temp1 !== null && temp1 !== "") && !IsBoolean(this.m_designMode) && (temp1.search("<html>")) === -1) {
		...
		if (this.m_textBoxText !== "") {
			if (this.m_textdecoration === "underline") {
				var u = document.createElement("u");
				box.innerHTML = "<u>" + this.m_textBoxText + "</u>";
				obj.appendChild(u);
			} else {
				box.innerHTML = this.m_textBoxText;
			}
		}
	} else {
		box.innerHTML = this.m_textBoxText;
...
//		console.log($(this).html());
		temp.m_textBoxText = $(this).html();
	});
	$("#draggableDiv" + temp.m_objectid).append(obj);
};
TextBox.prototype.getDataPointAndUpdateGlobalVariable = function () {
