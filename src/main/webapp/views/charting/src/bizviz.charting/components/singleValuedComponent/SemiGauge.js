 ...
 * @File: SemiGauge.js
 * @description SemiGauge
 **/
function SemiGauge(m_chartContainer, m_zIndex) {
	this.base = Widget;
	this.base();

	/** Creating Global variables which will be used in Processing and creating object of SemiGaugeMovement() class and ChartFrame() class**/
	this.m_requiredData = "";
	this.value = [];
	...
	this.m_speedometerwidth = 10;
	this.m_needlewidth = 3;
	...
	this.m_fontfamily = "";
	this.m_fontcolor = "";
	this.m_markerfontfamily = "";
	this.m_markerfontsize = "";
	this.m_markerfontcolor = "";
	...
	this.m_niddleMovement = new SemiGaugeMovement(this);
	this.m_chartFrame = new ChartFrame();
};

/** @description Using prototype inheriting the variable and method of Widget into Gauge**/
SemiGauge.prototype = new Widget();

/** @description This method will parse the chart JSON and create a container **/
SemiGauge.prototype.setProperty = function (chartJson) {
	this.ParseJsonAttributes(chartJson.Object, this);
	this.initCanvas();
};

/** @description Getter for Object Type **/
SemiGauge.prototype.getObjectName = function () {
	return this.getObjectType();
};

/** @description Iterate through chart JSON and set class variable values with JSON values **/
SemiGauge.prototype.ParseJsonAttributes = function (jsonObject, nodeObject) {
...
SemiGauge.prototype.setDataProvider = function (data) {
...
SemiGauge.prototype.initializeDraggableDivAndCanvas = function (dashboardName, zindex) {
...
SemiGauge.prototype.setDashboardNameAndObjectId = function () {
...
/** @description Starting of SemiGauge Drawing **/
SemiGauge.prototype.draw = function () {
	this.drawObject();
};

/** @description Calling init and drawChart function **/
SemiGauge.prototype.drawObject = function () {
	this.init();
	this.drawChart();
...
/** @description SemiGauge initialization **/
SemiGauge.prototype.init = function () {
	...
	} else if (this.m_valuevariable !== "" && this.updateFlag === false) {
...
SemiGauge.prototype.drawChart = function () {
...
SemiGauge.prototype.getDataPointAndUpdateGlobalVariable = function () {
...
SemiGauge.prototype.getBgGradients = function () {
	return this.m_bggradients;
};

/** @description Getter for Background Alpha **/
SemiGauge.prototype.getBGAlpha = function () {
	return this.m_bgalpha;
};

/** @description Setter for Background Alpha **/
SemiGauge.prototype.setBGAlpha = function (m_bgalpha) {
	this.m_bgalpha = m_bgalpha;
};
//------------------------------------all calculation part----------------------------------------------------------

/** @description Creating SemiGaugeMovement class and creating object of DrawSemiGauge and SemiGaugeText**/
function SemiGaugeMovement() {
	this.m_newDegrees = 0;
	this.m_degrees = 0;
	this.m_difference = 0;
	this.valueInDegree;
	...
	this.m_data;
	this.m_targetvalue;
	this.m_drawGauge = new DrawSemiGauge();
	this.m_gaugeText = new SemiGaugeText();
};

/** @description Movement class initialization **/
SemiGaugeMovement.prototype.init = function (gaugeRef) {
...
SemiGaugeMovement.prototype.draw = function () {
	this.generateDegree();
...
SemiGaugeMovement.prototype.generateDegree = function () {
...
SemiGaugeMovement.prototype.getValue = function () {
...
SemiGaugeMovement.prototype.getTargetValue = function () {
...
SemiGaugeMovement.prototype.animate_to = function () {
...
/** @description Creation of DrawSemiGauge Class**/
...
/** @description DrawSemiGauge initialization  **/
DrawSemiGauge.prototype.init = function (niddleMovementRef) {
...
DrawSemiGauge.prototype.drawWithNewDegree = function (degrees, valueInDegree, targetInDegree) {
...
DrawSemiGauge.prototype.Outercircle = function () {
	this.maxvalue = this.m_niddleMovementRef.m_gaugeRef.m_maxvalue;
	var rad = this.ctx.createLinearGradient(this.m_x, this.m_y, this.m_x, parseInt(this.m_y) + parseInt(this.m_height));
	var color1 = convertColorToHex(this.m_niddleMovementRef.m_gaugeRef.m_bezelcolor);
	var color2 = hex2rgb(color1, 0.1);
	rad.addColorStop(0, color1);
	rad.addColorStop(0.3, color2);
	rad.addColorStop(0.7, color2);
	rad.addColorStop(1, color1);

	var rad1 = this.ctx.createLinearGradient(this.m_x, this.m_y + 18, this.m_x, parseInt(this.m_y) + parseInt(this.m_height) - 20);
	var color11 = convertColorToHex(this.m_niddleMovementRef.m_gaugeRef.m_faceshadowcolor);
	var color12 = hex2rgb(color11, 0.1);
	rad1.addColorStop(0, color11);
	rad1.addColorStop(0.3, color12);
	rad1.addColorStop(0.7, color12);
	rad1.addColorStop(1, color11);
	this.drawRatioMeter();
};

/** @description Creating Ratio Meter which will draw with 240 degree only**/
DrawSemiGauge.prototype.drawRatioMeter = function () {
	var startangle = [];
	var endAngle = [];
	...
	for (var i = 0; i < this.ratio.length; i++) {
		if (i == 0)
			startangle[i] = Math.PI - (30 * Math.PI / 180);
		else
			startangle[i] = endAngle[i - 1];

		if (i == (this.ratio.length - 1))
			endAngle[i] = 2 * Math.PI + (30 * Math.PI / 180);
		else
			endAngle[i] = ((this.getRatioValue(this.ratio[i] * 1)) * eachNumberIndegree + 150) * Math.PI / 180;
		this.drawCircle(this.m_x * 1 + this.m_width / 2 * 1, this.m_y * 1 + this.m_height / 2 * 1, this.m_radius - this.m_niddleMovementRef.m_gaugeRef.m_speedometerwidth / 2, startangle[i], endAngle[i], false, this.m_colors[i], this.m_niddleMovementRef.m_gaugeRef.m_speedometerwidth);
	}
};

/** @description Calculate Ratio **/
DrawSemiGauge.prototype.getRatioValue = function (ratio) {
...
DrawSemiGauge.prototype.drawCircle = function (centeX, centerY, m_radius, startAngle, endAngle, dir, strokeStyle, lineWidth) {
	this.ctx.save();
	this.ctx.beginPath();
...
DrawSemiGauge.prototype.Innercircle = function () {
	...
	this.drawCircle(this.m_x * 1 + this.m_width / 2 * 1, this.m_y * 1 + this.m_height / 2 * 1, this.m_radius / 7, 0, Math.PI * 2, false, color, null);
};

/** @description Creating Watch Stick **/
DrawSemiGauge.prototype.drawWatchStick = function () {
...
DrawSemiGauge.prototype.drawWatchStickMarker = function (angle) {
	this.ctx.beginPath();
	this.ctx.lineWidth = (this.m_radius * 2) / 100;
	this.ctx.strokeStyle = "";
	var stickStartPoint = this.m_radius - this.m_niddleMovementRef.m_gaugeRef.m_speedometerwidth * 1; // it is same as the start point of the alerts color.
	var stickEndPoint = stickStartPoint + this.m_niddleMovementRef.m_gaugeRef.m_speedometerwidth * 1; // " this.m_radius/5 " is the width of the alerts color
	this.ctx.strokeStyle = convertColorToHex(this.m_niddleMovementRef.m_gaugeRef.m_tickscolor);
	this.ctx.moveTo(this.m_x * 1 + this.m_width / 2 * 1 + (stickStartPoint) * Math.cos(angle), (stickStartPoint) * Math.sin(angle) + this.m_y * 1 + this.m_height / 2 * 1);
	this.ctx.lineTo(this.m_x * 1 + this.m_width / 2 * 1 + (stickEndPoint) * Math.cos(angle), (stickEndPoint) * Math.sin(angle) + this.m_y * 1 + this.m_height / 2 * 1);
	this.ctx.stroke();
	this.ctx.closePath();
};

/** @description Creating Target Mark on the Ratio Meter**/
DrawSemiGauge.prototype.drawTarget = function () {
	if (this.m_targetValueInDegree !== "" && !isNaN(this.m_targetValueInDegree) && this.m_niddleMovementRef.m_gaugeRef.m_targetvalue !== "") {
		if (this.m_niddleMovementRef.minvalue * 1 <= this.m_niddleMovementRef.m_gaugeRef.m_targetvalue * 1 && this.m_niddleMovementRef.maxvalue * 1 >= this.m_niddleMovementRef.m_gaugeRef.m_targetvalue * 1) {
			...
			var stickStartPoint = this.m_radius - this.m_niddleMovementRef.m_gaugeRef.m_speedometerwidth * 1;
			var stickEndPoint = stickStartPoint + this.m_niddleMovementRef.m_gaugeRef.m_speedometerwidth * 1;
			/*this.ctx.beginPath();
			this.ctx.save();
			//this.ctx.setLineDash([2,2]);
			//this.ctx.lineDashOffset=2;
			this.ctx.lineWidth=(this.m_radius*1.5)/100;
			this.ctx.strokeStyle = convertColorToHex(this.m_niddleMovementRef.m_gaugeRef.m_targetcolor);
			this.ctx.translate(0.5,0.5);
			this.ctx.moveTo(this.m_x*1 + this.m_width/2*1 + (stickStartPoint)*Math.cos(targetAngle), (stickStartPoint)*Math.sin(targetAngle)+ this.m_y*1 + this.m_height/2*1);
			this.ctx.lineTo(this.m_x*1 + this.m_width/2*1 + (stickEndPoint)*Math.cos(targetAngle), ( stickEndPoint)*Math.sin(targetAngle)+ this.m_y*1 + this.m_height/2*1);
			this.ctx.stroke();
			this.ctx.restore();
			this.ctx.closePath();*/
			...
			var t1X = this.m_x * 1 + this.m_width / 2 * 1 + (stickEndPoint) * Math.cos(t1Angle);
			var t1Y = (stickEndPoint) * Math.sin(t1Angle) + this.m_y * 1 + this.m_height / 2 * 1;
			var t2X = this.m_x * 1 + this.m_width / 2 * 1 + (stickEndPoint) * Math.cos(t2Angle);
			var t2Y = (stickEndPoint) * Math.sin(t2Angle) + this.m_y * 1 + this.m_height / 2 * 1;
			var stickMidPoint = stickStartPoint + this.m_niddleMovementRef.m_gaugeRef.m_speedometerwidth * 1 / 2;
			var midX = this.m_x * 1 + this.m_width / 2 * 1 + (stickMidPoint) * Math.cos(targetAngle);
			var midY = (stickMidPoint) * Math.sin(targetAngle) + this.m_y * 1 + this.m_height / 2 * 1;
...
DrawSemiGauge.prototype.drawneedle = function () {
	var initialspeedometerwidth = 12;
	var x = ((this.m_radius - initialspeedometerwidth - this.m_niddleMovementRef.m_gaugeRef.m_needlewidth) * (Math.cos(this.m_newValueInDegree * Math.PI / 180)) + this.m_x * 1 + this.m_width / 2 * 1);
	var y = ((this.m_radius - initialspeedometerwidth - this.m_niddleMovementRef.m_gaugeRef.m_needlewidth) * (Math.sin(this.m_newValueInDegree * Math.PI / 180)) + this.m_x * 1 + this.m_height / 2 * 1);

	var x1 = this.m_x * 1 + this.m_width / 2 * 1;
	var y1 = this.m_x * 1 + this.m_height / 2 * 1;
	this.ctx.beginPath();
	this.ctx.moveTo(x, y);
	this.ctx.lineTo(x1, y1);
	this.ctx.lineWidth = this.m_niddleMovementRef.m_gaugeRef.m_needlewidth;
...
/** @description Creating SemiGaugeText Text Class**/
function SemiGaugeText() {
	this.m_degrees = "";
	this.m_xArray = [];
	this.m_maxvalue = [];
	this.m_minvalue = [];
...
SemiGaugeText.prototype.init = function (niddleMovementRef) {
	this.ctx = niddleMovementRef.m_gaugeRef.ctx;
	...
	this.m_valuefontcolor = this.m_niddleMovementRef.m_gaugeRef.m_fontcolor;
	this.m_valuefontsize = this.m_niddleMovementRef.m_gaugeRef.fontScaling(this.m_niddleMovementRef.m_gaugeRef.m_fontsize * 1);
	this.m_valuefontfamily = selectGlobalFont(this.m_niddleMovementRef.m_gaugeRef.m_fontfamily);

	this.m_markerfontcolor = convertColorToHex(this.m_niddleMovementRef.m_gaugeRef.m_markerfontcolor);
	this.m_markerfontsize = this.m_niddleMovementRef.m_gaugeRef.fontScaling(this.m_niddleMovementRef.m_gaugeRef.m_markerfontsize * 1);
	this.m_markerfontfamily = selectGlobalFont(this.m_niddleMovementRef.m_gaugeRef.m_markerfontfamily);

	this.m_fontColor = convertColorToHex(this.m_valuefontcolor);
};

/** @description Drawing of Value Text**/
SemiGaugeText.prototype.drawText = function (degrees, data) {
	var temp = this;
	this.ctx.save();
	this.ctx.beginPath();
	this.m_degrees = (this.m_niddleMovementRef.m_gaugeRef.m_maxvalue >= 100) ? degrees * x * 1.08 : degrees;
	
	var id = "showValue" + this.m_objectid;
	var originalVal = data * 1;
	
	var width = (this.m_niddleMovementRef.m_drawGauge.m_radius * 2 / 5) * 2;
	var height = this.m_niddleMovementRef.m_drawGauge.m_radius * 1 / 4;
	var x = this.m_x * 1 + this.m_width / 2 * 1 - width / 2;
	var y = this.m_y * 1 + this.m_height / 2 + this.m_niddleMovementRef.m_drawGauge.m_radius / 4;
	var fontFamily = this.m_niddleMovementRef.m_gaugeRef.m_fontfamily;

	var valueBox = this.setBox(x, y, id, width, height, data, fontFamily, this.m_valuefontsize, this.m_valuefontcolor, data);
	...
	var targetBox = this.setBox(x, (y + width / 2) - 5, targetId, width, height, targetVal, fontFamily, this.m_niddleMovementRef.m_gaugeRef.m_markerfontsize, this.m_niddleMovementRef.m_gaugeRef.m_targetcolor, "Target Value");
	...
	if (IsBoolean(this.m_niddleMovementRef.m_gaugeRef.m_showtitle)) {
		...
		var titleBox = this.setBoxForTitle(x, y, titleId, width, height, titleText, titlefontfamily, titlefontsize, titlefontColor);
...
/** @description Creating Box inside which Title will display**/
SemiGaugeText.prototype.setBoxForTitle = function (x, y, id, width, height, data, fontfamily, fontsize, fontcolor, tooltipData) {
	$("#" + id).remove();
...
SemiGaugeText.prototype.setBox = function (x, y, id, width, height, data, fontfamily, fontSize, fontcolor, tooltipData) {
	var fontsize = (fontSize); //*this.fontratio;
	var num = data;
	var formattedData = this.textformatter(num);
	$("#" + id).remove();
	var obj = document.createElement("div");
	obj.setAttribute("id", id);
	obj.style.position = "absolute";
	...
	valueObj.style.width = width + "px";
	valueObj.style.height = height + "px";
	valueObj.style.fontSize = (fontsize) + "px";
	valueObj.style.fontFamily = selectGlobalFont(fontfamily);
	valueObj.style.color = convertColorToHex(fontcolor);
	valueObj.style.verticalAlign = "middle";
	valueObj.style.textAlign = "center";
	valueObj.style.display = "TABLE-CELL";
	valueObj.style.position = "relative";
	valueObj.innerHTML = formattedData;
	obj.appendChild(valueObj);
	obj = document.body.appendChild(obj);
...
SemiGauge.prototype.initMouseAndTouchEvent = function (comp) {
...
SemiGaugeText.prototype.getFontSizeAccordingWidth = function () {
...
SemiGaugeText.prototype.radiobuttonPosition = function () {
...
SemiGaugeText.prototype.drawMinRadioButton = function () {
	this.setMintextPos();
	this.drawMinText(this, this.m_minvalue, this.m_objectid + "minradio");
};

/** @description Calculating Min Text Value position**/
SemiGaugeText.prototype.setMintextPos = function () {
	var temp = this;
	var stickStartPoint = this.m_niddleMovementRef.m_drawGauge.m_radius; // it is same as the start point of the alerts color.
	var left = this.m_x * 1 + this.m_width / 2 * 1 + (stickStartPoint) * Math.cos(150 * Math.PI / 180);
	var top = this.m_y * 1 + this.m_height / 2 * 1 + (stickStartPoint) * Math.sin(150 * Math.PI / 180);
...
SemiGaugeText.prototype.drawMinText = function (thisradio, val, id) {
	var temp = this;
	var newtext = this.textformatter(val);
	document.getElementById("mintext" + this.m_objectid).innerHTML = "<font style='color:" + temp.m_markerfontcolor + ";font-size:" + temp.m_markerfontsize + "px;font-family:" + selectGlobalFont(temp.m_markerfontfamily) + ";'>" + " " + newtext + "</font>";
};

/** @description Drawing of Max Radio Button**/
SemiGaugeText.prototype.drawMaxRadioButton = function () {
...
SemiGaugeText.prototype.setMaxtextPos = function () {
	var temp = this;
	var stickStartPoint = this.m_niddleMovementRef.m_drawGauge.m_radius; // it is same as the start point of the alerts color.
	var right = this.m_x * 1 + this.m_width / 2 * 1 - (stickStartPoint) * Math.cos(30 * Math.PI / 180);
	var top = this.m_y * 1 + this.m_height / 2 * 1 + (stickStartPoint) * Math.sin(30 * Math.PI / 180);
...
SemiGaugeText.prototype.Maxvalue = function (thisradio, val, id, x) {
	var temp = this;
	var newtext = this.textformatter(val);
	$("#maxtext" + this.m_objectid).css("text-align", "right");
	document.getElementById("maxtext" + this.m_objectid).innerHTML = "<font style='color:" + temp.m_markerfontcolor + ";font-size:" + temp.m_markerfontsize + "px;font-family:" + selectGlobalFont(temp.m_markerfontfamily) + ";'>" + " " + newtext + "</font>";
};

/** @description Creating Div and returning div object**/
SemiGaugeText.prototype.createDiv = function (x, y, id) {
	var width = this.m_niddleMovementRef.m_drawGauge.m_radius * 7 / 10;
	var height = this.m_niddleMovementRef.m_drawGauge.m_radius * 1 / 6;
	$("#" + id).remove();
	var obj = document.createElement("div");
	obj.setAttribute("id", id);
	obj.style.position = "absolute";
...
SemiGaugeText.prototype.textformatter = function (text) {
	this.setFormatter();
	this.setSecondaryFormatter();	
	return this.getFormattedText(text);
};

SemiGaugeText.prototype.setFormatter = function () {
	this.m_unitSymbol = "";
...
SemiGaugeText.prototype.setSecondaryFormatter = function () {
...
SemiGaugeText.prototype.getFormattedText = function (text) {
...
SemiGaugeText.prototype.setPrecision = function (text, precision) {
	if(text !== 0){
		if(precision !== "default") {
...
SemiGaugeText.prototype.getFormatterSymbol = function (formatter, unit) {
	var unitSymbol = "";
	try {
...
SemiGaugeText.prototype.getCurrencySymbol = function (unit) {
...
SemiGaugeText.prototype.getNumberSymbol = function (unit) {
	switch (unit.toLowerCase()) {
	case "percent":
		return "%";
	case "auto":
...
SemiGaugeText.prototype.updateTextWithFormatter = function (text, unit, precision) {
	var value = text;
	switch (unit) {
	case "K":
		value = text * 1 / 1000;
...
SemiGaugeText.prototype.addSecondaryFormater = function (text, secondaryUnitSymbol) {
	var textValue = text;
...
SemiGaugeText.prototype.getSecondaryFormaterAddedText = function (textValue, secondaryUnitSymbol) {
...
SemiGaugeText.prototype.addUnitAssuffix = function (text, unitSymbol) {
	return text + unitSymbol;
};
SemiGaugeText.prototype.addUnitAsprefix = function (text, unitSymbol) {
	return unitSymbol + text;
};
SemiGaugeText.prototype.addFormatter = function (text, unitSymbol, signPosition) {
	try {
