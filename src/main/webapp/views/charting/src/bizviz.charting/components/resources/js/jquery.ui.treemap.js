...
TreemapUtils.changeColor = function (color, ratio, darker) {
	...
	[
		'#',
		Math[darker ? 'max' : 'min'](
			parseInt(decimal[0], 10) + difference, darker ? 0 : 255).toString(16).lpad("0", 2),
		Math[darker ? 'max' : 'min'](
			parseInt(decimal[1], 10) + difference, darker ? 0 : 255).toString(16).lpad("0", 2),
		Math[darker ? 'max' : 'min'](
			parseInt(decimal[2], 10) + difference, darker ? 0 : 255).toString(16).lpad("0", 2)
	].join('');
};
...
TreemapUtils.squarify = function (rect, vals) {
	...
	if (rect[2] <= 0 || rect[3] <= 0) {
		...
		newVals = vals.map(function (item) {
				return item * (rect[2] * rect[3]);
			});
...
(function ($) {
	$.widget("ui.treemap", {
		...
		_renderNodes : function () {
			if (this._animationActive !== undefined && this._animationActive === true)
				return;
			var processNodes = function (nodes) {
				var bodyRect,
				headerRect,
					...
					if (that._isRootNode(nodes[i]) === false) { // skip root node
						...
						if (nodes[i].hasOwnProperty('children') && headerRect !== null) { // group node
							...
							if (nodes[i].hasOwnProperty('children') && that.options.nodeBorderWidth === 0) {
								ctx.strokeStyle = that.options.leafNodeBodyStroke.call(that, ctx, bodyRect, nodes[i].computedColor);
								ctx.lineWidth = that.options.leafNodeBodyStrokeLineWidth;
								ctx.beginPath();
							...
							ctx.strokeStyle = that.options.leafNodeBodyStroke.call(that, ctx, bodyRect, nodes[i].computedColor);
							ctx.lineWidth = that.options.leafNodeBodyStrokeLineWidth;
							// alert(bodyRect[0]+"==="+bodyRect[1]+"===="+bodyRect[2]+"==="+bodyRect[3]);
				...
				for (i = 0; i < nodes.length; i++) {
					if (that._isRootNode(nodes[i]) === false) { // skip root node
						...
						if (nodes[i].hasOwnProperty('children')) {
							if (headerRect !== null) {
								// Inner Node
								that.options.innerNodeHeaderLabeller.call(that, ctx, headerRect, nodes[i].computedColor, nodes[i].name);
							}
						} else {
							// Leaf Node
							that.options.leafNodeBodyLabeller.call(that, ctx, bodyRect, nodes[i].computedColor, nodes[i].name, nodes[i].AdditionalDataLabel);
							that.stats['leafNodeCnt'] += 1;
		...
		_refreshCanvas : function () {
			...
			this.element.append(canvas).mousemove(function (e) {
				if (that._animationActive !== undefined && that._animationActive === true)
					return;
				...
				if (e.pageX < offsetX + width && e.pageY < (offsetY + height)) {
					ids = that._coordsToId(parseInt(e.pageX - offsetX), parseInt(e.pageY - offsetY));
					nodes = [];
				...
				if(!(that.options.desktopView)){
				    ...
				    if (that.options.singleClickTimer == null) {
				        /**Added to enable drill on double tap on mobile.*/
				        that.options.singleClickTimer = setTimeout(function() {
				            that.options.singleClickTimer = null;
				        }, that.options.doubletapTimeout);
				    } else {
				        var offset = that.element.offset(),
				            offsetX = parseInt(offset.left, 10), // offsets are float values on mac/FF
				            offsetY = parseInt(offset.top, 10), // convert them to ints so coordsToId will work
				            ...
				            data,
				            i,
				            PageTop = e.originalEvent.touches[0].pageY,
				            PageLeft = e.originalEvent.touches[0].pageX;
				        clearTimeout(that.options.singleClickTimer);
				        that.options.singleClickTimer = null;
				        if (PageLeft < offsetX + width && PageTop < (offsetY + height)) {
				            ids = that._coordsToId(parseInt(PageLeft - offsetX), parseInt(PageTop - offsetY));
				            nodes = [];
				            for (i = 0; i < ids.length; i++) {
				                nodes.push(that._getNode([ids[i]]));
				            }
				            ...
				            that._trigger('touchstart', e, data);
				        }
				    }
				}
			});
