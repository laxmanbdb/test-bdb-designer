 ...
 * 
 * @path: Certificate of jQuery EasyUI.pdf
 * @text: This license agreement refers to jQuery EasyUI software - Commercial License.
...
(function ($) {
	...
	function _d7(_d8) {
		...
		$(_d8).unbind().bind("mouseover", function (e) {
			...
			/** Added for provide custom child node selection in hfilter*/ 
			var opts = $(this).tree("options");
			if (opts.multiple && opts.temp.m_hierarchylevel == "Multiple" && !IsBoolean(opts.temp.m_controlledselection) && IsBoolean(opts.temp.m_customselection) && opts.temp.isSingleChild && opts.temp.m_selectionlimit < (getSelectedNodeLength(e, this, opts))) {
			    e.stopPropagation();
			    if (tt.find("span").hasClass("child-node")) {
			        _da.addClass("tree-node-hover-notallowed");
			    }
			} else {
			    if (tt.hasClass("tree-node-hover-notallowed")) {
			        tt.removeClass("tree-node-hover-notallowed")
			    }
		    ...
		    /** Added for provide custom child node selection in hfilter @khivraj.*/
		    var opts = $(this).tree("options");
		    var tt = $(e.target);
		    ...
		    if (opts.multiple && opts.temp.m_hierarchylevel == "Multiple" && !IsBoolean(opts.temp.m_controlledselection) && IsBoolean(opts.temp.m_customselection) && opts.temp.isSingleChild && opts.temp.m_selectionlimit < (getSelectedNodeLength(e, this, opts))) {
		        ...
		        // Existing code before "custom child node selection" feature implementation 
		        if (tt.hasClass("tree-hit")) {
		            _13b(_d8, _dc[0]);
		            return false;
		        } else {
	...
	/** Added to return unchecked node length of tree  @khivraj*/ 
	function getUncheckedIndex(node, childNodeCheckedIndex) {
		var childNodeUnCheckedIndex = [];
		node.each(function(){
			var id = $(this).attr("index").split("node_")[1];
			if (childNodeCheckedIndex.indexOf(id*1) < 0){
				childNodeUnCheckedIndex.push(id*1);
			}
		});
		return childNodeUnCheckedIndex.length;
	};
	/**Added for provide custom child node selection in hfilter @khivraj*/ 
	function getSelectedNodeLength(e, temp, opts) {
		var tt = $(e.target);
		var _dc = tt.closest("div.tree-node");
	    var _974 = $(temp).tree("getChecked");
	    var parentNodes = $.data(temp, "tree").data;
	    var rootNodeIndexs = [];
	    for (_j = 0; _j < parentNodes.length; _j++) {
	        rootNodeIndexs.push(parentNodes[_j].id);
	        //Removing root elements from checked element list
	        var _index = _974.indexOf(parentNodes[_j]);
	        if (_index > -1) {
	            _974.splice(_index, 1);
	        }
	    }
	    var childNodeCheckedIndex = [];
	    for (var _k = 0; _k < _974.length; _k++) {
	        childNodeCheckedIndex.push(_974[_k].id);
	    }
	    var elementId = (_dc.attr("index")) ? _dc.attr("index").split("node_")[1] : "";
	    var isRootNodeEmpty = (_dc.siblings("ul").length > 0 && _dc.siblings("ul").find("li").length > 0) ? false : true; // return rootNode has child node or not
	    var childNodeLength = (isRootNodeEmpty) ? 0 : ((!tt.hasClass("tree-hit")) ? getUncheckedIndex(_dc.siblings("ul").find("li").children(), childNodeCheckedIndex) : 0); // return rootNode child length
	    var length = (!tt.hasClass("tree-hit") && !tt.hasClass("tree-checkbox1") && (rootNodeIndexs.indexOf(elementId * 1) < 0)) ? ((_df(temp, _dc[0]) && _df(temp, _dc[0]).checked) ? _974.length : _974.length + 1) : _974.length; //target node checked or unchecked 
	    length = (!tt.hasClass("tree-hit") && tt.hasClass("tree-checkbox1") && (rootNodeIndexs.indexOf(elementId * 1) > -1)) ? length : (length + childNodeLength);
	...
	function _133(_134, _135, _136) {
		...
		if (hit.hasClass("icons bd-minus")) {
			return;
		}
		...
		hit.removeClass("icons bd-plus").addClass("icons bd-minus");
	...
	function _138(_139, _13a) {
		...
		hit.removeClass("icons bd-minus").addClass("icons bd-plus");
		...
		render : function (_1b5, ul, data) {
			...
			function _1b7(_1b8, _1b9) {
				...
				for (var i = 0; i < _1b9.length; i++) {
					...
					cc.push("<li>");
					cc.push("<div id=\"" + item.domId + "\" class=\"tree-node\" index=\"node_" + item.id + "\">");
					for (var j = 0; j < _1b8; j++) {
						cc.push("<span class=\"tree-indent\"></span>");
					}
					var _1ba = false;
					/**Added for BDD-658 : Hierarchical Combo: UI update for Quantiphi.*/
					if ((opts.advanceTheme !== undefined) && (opts.advanceTheme ==  "advance")) {
						if (item.state == "closed") {
					        cc.push("<span class=\"tree-hit icons bd-plus\"></span>");
					        cc.push("<span class=\"tree-icon tree-folder " + (item.iconCls ? item.iconCls : "") + "\"></span>");
					    } else {
					        if (item.children && item.children.length) {
					            cc.push("<span class=\"tree-hit icons bd-minus\"></span>");
					   ...
					   /**BDD:700-Added to align child field center*/
					        if (item.children && item.children.length) {
					        	cc.push("<span class=\"tree-title parent-node\">" + opts.formatter.call(_1b5, item) + "</span>");
					        } else {
					        	cc.push("<span class=\"tree-title-child child-node\">" + opts.formatter.call(_1b5, item) + "</span>");
					            _1ba = true;
					        }
					    if (opts.checkbox) {
					        if ((!opts.onlyLeafCheck) || _1ba) {
					        	if(opts.showCheckBox){
					        		cc.push("<span class=\"tree-checkbox tree-checkboxPosition tree-checkbox0\"></span>");
					        	}else{
					        		if(item.children && item.children.length){
						            	/**Do Nothing:: BDD:700-Added to hide parent check boxes*/
						            }else{
						            	cc.push("<span class=\"tree-checkbox tree-checkboxPosition tree-checkbox0\"></span>");
						            }
					        	}
					    ...
					    if (item.children && item.children.length) {
				        	...
				        	cc.push("<span class=\"tree-title child-node\">" + opts.formatter.call(_1b5, item) + "</span>");
				        }
...
(function ($) {
	...
	function _5b0(_5b1) {
		...
		function _5b6(_5bb, _5bc, _5bd) {
			...
			for (var i = 0; i < _5bc.length; i++) {
				if (_5bc[i][0]._field != " " && _5bc[i][0].title != " ") {
	...
	function _5f5(_6c3, _6c4) {
		...
		setTimeout(function () {
			// no need to show loader on grid
			//_6c6();
		}, 0);
		function _6c6() {
	...
	var _728 = {
		render : function (_729, _72a, _72b) {
			...
			if ((opts.sortName !== null)&&(opts.isNumericArrMap && opts.isNumericArrMap[opts.sortName]=="Numeric")) {
				/**Added by Swapnil Shrivastava, to support comma saprated and garbage value when column type is Numeric*/
			    for (var i = 0; i < opts.data.length; i++) {
			        if (!Array.isArray(opts.data[i])) {
			            emptyObj.push(opts.data[i]);
			        } else {
			            if ((opts.data[i].gridHiddenField !== "aggregatedRow")&&(!isNaN(getCommaRemovedValue(opts.data[i][opts.sortName])))&&(opts.data[i][opts.sortName]!=="")) {
			                rowsArr.push(opts.data[i]);
			            } else if(opts.data[i].gridHiddenField == "aggregatedRow"){
			                isAggregatedRow = true;
			                aggregatedRow = opts.data[i]
			            }else{
			            	isStringValue = true;
			            	stringObjArray.push(opts.data[i]);
			            }
			        }
			        if (Array.isArray(_72c.data.rows[i]) && (_72c.data.rows[i].gridHiddenField !== "aggregatedRow")) {
			            rowsSortArray.push(_72c.data.rows[i]);
			        }
			    }
			    if (isStringValue) {
			    	for (var l = 0; l < stringObjArray.length; l++) {
				        rowsArr.push(stringObjArray[l]);
				        /**To avoid the duplicacy of row*/
				        if(rowsSortArray.length < opts.data.length){
				        	rowsSortArray.push(stringObjArray[l]);
				        }
				    }
			    }
			    if (isAggregatedRow) {
			        rowsArr.push(aggregatedRow);
			        rowsSortArray.push(aggregatedRow);
			    }
			    for (var k = 0; k < emptyObj.length; k++) {
			        rowsArr.push(emptyObj[k]);
			        rowsSortArray.push(emptyObj[k]);
			    }
			    rows = rowsArr;
			    RowsSorted = [];
			    /**Added to match sorted data Array with actual data*/
				for (var j = 0; j < rowsSortArray.length; j++) {
				    for (var k = 0; k < rows.length; k++) {
				        if (rowsSortArray[j].gridHiddenField == rows[k].gridHiddenField) {
				        	RowsSorted[j] = rows[k];
				            break;
				        }        
				    }
				...
				rows = RowsSorted;
			}else if(opts.isEmptyRowsAdded){
				/**Added by Kheenvraj, to arrange column values for empty rows and aggregated row when column type is None*/
                for(var i=0; i< _72c.data.rows.length;i++) {
                	if(!Array.isArray(_72c.data.rows[i])) {
                		emptyObj.push(_72c.data.rows[i]);
                	} else {
                		if(_72c.data.rows[i].gridHiddenField !== "aggregatedRow") {
                			rowsArr.push(_72c.data.rows[i]);
                		}
                		else {
                			isAggregatedRow = true;
                			aggregatedRow = _72c.data.rows[i]
                		}
                	}
                }
                if(isAggregatedRow){
	...
	$.fn.datagrid.defaults = $.extend({}, $.fn.panel.defaults, {
			...
			rowStyler : function (_76a, _76b) {
				/** Added for append css class in datagrid aggregatedRow */
				if (_76b && _76b["gridHiddenField"] == "aggregatedRow") {
					return {'class' : "aggregatedRow"};
				}
			},
...
(function ($) {
	...
	var _88b = $.extend({}, $.fn.datagrid.defaults.view, {
			render : function (_88c, _88d, _88e) {
				...
				function _892(_893, _894, _895) {
					...
					for (var i = 0; i < _895.length; i++) {
						...
						_898 = _898.concat(view.renderRow.call(view, _88c, _88f, _893, _894, row, _89c));
						_898.push("</tr>");
			...
			renderRow : function (_8a2, _8a3, _8a4, _8a5, row, _tdId) {
				...
				for (var i = 0; i < _8a3.length; i++) {
					...
					if (col) {
						var css = col.styler ? (col.styler(row[_8a6], row, col.field, col, _tdId) || "") : "";
						var _8a7 = "";
						var _8a8 = "";
						if (typeof css == "string") {
							_8a8 = css;
						...
						if (col.checkbox) {
							...
							var val = null;
							if (col.formatter) {
								val = col.formatter(row[_8a6], row, col.field, col, _tdId);
							} else {
								val = row[_8a6];
							}
							if (_8a6 == opts.treeField) {
								...
								if (opts.rowFontProperties != undefined && !opts.nowrap) {
									var properties = opts.rowFontProperties;
									cc.push("<span class=\"tree-title\" style='white-space:normal;height:auto;font-size:"+properties.fontsize+";font-family:"+properties.fontfamily+";font-weight:"+properties.fontweight+";font-style:"+properties.fontstyle+";'>" + val + "</span>");
								}else{
	...
	function _8ec(_8ed) {
		...
		if (_8f1.closed) {
			_8f0.panel("panel").show().css({
				zIndex : ($.fn.menu ? (($.fn.menu.defaults) ? $.fn.menu.defaults.zIndex++ : 0) : $.fn.window.defaults.zIndex++),
				left : -999999
			});
			_8f0.panel("resize", {
...
(function ($) {
	function create(target) {
		...
		$(target).treegrid($.extend({}, opts, {
				...
				loadFilter : function (data, parentId) {
					...
					opts.frozenColumns[0][0].width = opts.frozenWidth;
		...
		$.map(opts.pivot.rows, function (field, index) {
			...
			if (pfield) {
				...
				$.map(groups, function (rows) {
					var r = sumR1(rows);
					r._rows = rows;
					var rowValue = (rows[0][field] == undefined) ? (" ") : (rows[0][field]);
					r[opts.treeField] = rowValue;
					r[opts.idField] = _idIndex++;
					topRows.push(r);
					allRows.push(r);
				});
			...
			function _sum(field) {
				...
				var rr = $.map(rows, function (row) {
						for (var i = 0; i < opts.pivot.columns.length; i++) {
							var value = row[opts.pivot.columns[i]];
							if (row[opts.pivot.columns[i]] == undefined)
								value = " ";
							if (value != col.tt[i]) {
								return undefined;
							}
	...
	$.fn.pivotgrid.defaults = $.extend({}, $.fn.treegrid.defaults, {
			...
			frozenColumns : [[{
						field : '_tree_field',
						width : $.fn.treegrid.defaults.frozenWidth,
						title : '',
						sortable : true
					}
				]],
			...
			operators : {
				sum : function (rows, field) {
					...
					var v = 0;
					$.map(rows, function (row) {
						/**Added below method to fix aggeration issue with comma separated values @BDD-776 */
						var val = getNumericComparableValue(row[field]);
						if (!isNaN(val) && val != "")
						v = v*1+(val)*1;
					});
					return v;
				...
				min : function (rows, field) {
					if(rows.length>0){
					/**Added below method to fix aggeration issue with comma separated values @BDD-776 */
					var min = parseFloat(getNumericComparableValue(rows[0][field]));
				...
				uniquevalue : function (rows, field) {
					var v = [];
					$.map(rows, function (row) {
						/**Added below method to fix aggeration issue with comma separated values @BDD-776 */
						var val = getNumericComparableValue(row[field]);
				...
				none : function (rows, field) {
					var v = [];
					var opts = $(this).pivotgrid('options');
					
					if(rows.length>0 && (rows[0][field] == undefined))
						return [[opts.placeholderNA]]; 
					$.map(rows, function (row) {
						if (row[field] != "")
							v.push((row[field]));
					});
...
/* JeasyUI Scroll View to load large data in simple datagrid */
...
var scrollview = $.extend({}, $.fn.datagrid.defaults.view, {
	...
	render: function(target, container, frozen){
		...
		//Added to resolve aggregatedRow position issue while sorting column when scrollView is enabled in SimpleGrid [BDD-792] @khivraj.
		if(opts && opts.view.type == "scrollview" && opts.pageSize > 0) {
			let aggregatedRowIndex = -1;
			aggregatedRowIndex = rows.findIndex(function(obj){return obj.gridHiddenField == "aggregatedRow";});
			if (aggregatedRowIndex > -1) {
				let aggregatedRow = rows.splice(aggregatedRowIndex, 1);//Removing
				rows.push(aggregatedRow[0]);// Adding on last index of rows arr
			}
		}
	...
	onBeforeRender: function(target){
		...
		function init(){
			...
			opts.onLoadSuccess = function(){
				/* Do not Call on load incase of scroll view , it will make recursive call to same method and breaks the code */
				//opts.onLoadSuccess.call(opts);
				opts.onAfterRender && opts.onAfterRender.call(opts);
			};
			setTimeout(function(){
		...
		function scrolling(){
			if (!opts.finder.getRows(target).length){
				...
				/* To load the css of grid when scrollview enabled */
				if (opts.onAfterRender) {
					//opts.onAfterRender.call(this);
				}
			}
		...
		return jq.each(function(){
			...
			cells.draggable({
				...
				proxy:function(source){
					var p = $('<div class="" style="position:absolute;border:1px solid #ff0000"/>').appendTo('body');
