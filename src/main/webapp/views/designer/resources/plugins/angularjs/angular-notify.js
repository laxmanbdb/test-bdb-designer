angular.module('cgNotify', []).factory('notify',['$timeout','$http','$compile','$templateCache','$rootScope',
    function($timeout,$http,$compile,$templateCache,$rootScope){
        ...
        var notify = function(args){
            ...
            args.duration = (args.duration >= 0) ? args.duration : defaultDuration;
            ...
            scope.$showClose = (args.classes == "alert-warning" || args.classes == "alert-danger") ? true : false;
            scope.$iconClass = 
            	{
	            	'alert-success': 'glyphicon-ok-sign', 
					'alert-info': 'glyphicon-info-sign', 
					'alert-warning': 'glyphicon-exclamation-sign', 
...
angular.module('cgNotify').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('angular-notify.html',
	...
	"    	<div>" +
	"    		<span>{{$message}}</span>" +
	"    		<button type=\"button\" class=\"cg-notify-close\" ng-click=\"$close()\" ng-show=\"$showClose\">\n" +
