...
function saveImageToDatabase(imagefinal) {
	$('#resizer-demo').html('').addClass("hide");
 	...
 	var data = '<div id="myModal1" role="dialog"><div class="modal-dialog capture-dialog"><div class="modal-content"><div class="modal-header" style="padding: 0px 15px;"><h2 class="modal-title">Save screenshot...</h2></div><div class="modal-body" style="padding: 0px 15px;"><img class="my-image" id = "imageCrp" src="'+imagefinal+'" alt="Picture"  style="max-width: 500px; max-height: 250px; image-rendering: -webkit-optimize-contrast;"><h3>Image Name</h3><input class ="inputField" type="text" id="imageName" name="imageName" placeholder="Give a name to screenshot.."><small id="imgNameError"></small><h3>Tags</h3><input class ="inputField" type="text" id="tagname" name="tagname" placeholder="Tags must be separated with space.."></div><div class="modal-footer"></div></div></div></div>';
    	...
    	if(imagename == ''){
		    ...
		    if(typeof req_url != 'undefined') {
				    parent.BIZVIZ.SDK.secureRequest(req_url.gallery.pluginService, reqData, function(data, status) {
						...
						if( window.parent.BIZVIZ ) {
							parent.$(".modal-body").empty();
							parent.$(".modal-body").html("<p class='model-message'>" + message + "</p>");
							parent.$(".modal-footer .saveImg").hide();
						} else if( window.BIZVIZ ){
							$(".modal-body").empty();
							$(".modal-body").html("<p class='model-message'>" + message + "</p>");
							$(".modal-footer .saveImg").hide();
						}
				     });
		    } else {
    ...
    if(document.getElementById('content-Div')) {
		...
		window.parent.document.getElementById('legacy-support').appendChild(modal);
	}
/*	var $image = window.parent.$('#imageCrp');
	var cropp = parent.$('.demo');
	$(cropp).croppie({
	    url: imagefinal,
	});
	$(parent.$('.my-image')).croppie();*/
